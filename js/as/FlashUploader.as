package
{
  ...
  */
    ...
    // flag to indicate that the file should be read locally
    private const LOCAL_READ:String = "_localread_";

    private var fileToReadRef:FileReference;
    private var fileParserType:String;
    ...
    // Browse for single or multiple files, with or without a filter array.
      ...
      // read the file locally if url is set to special flag
      if (url === LOCAL_READ) {
        readFile(fr, vars);

      } else {
        // submit the file upload request
    ...
    /**
     * Encapsulate logic for creating parsers for different types of files.
     * Current implementation only process text files.
     */

    private function fileParserFactory(parserType:String, fileRef:FileReference):IFileParser {
        var parser:IFileParser = null;

        if (parserType === "textfile") {
            parser = new TextFileParser(fileRef);
        } else {
    ...
    private function readFile(fileToReadRef:FileReference, readerCfg:Object):void {
        this.fileToReadRef = fileToReadRef;
        this.fileParserType = readerCfg.parser;

        fileToReadRef.addEventListener(Event.COMPLETE, fileReadComplete);
        fileToReadRef.load();
    }

    private function fileReadComplete(evt:Event):void {
        try {
          var parser:IFileParser =
              fileParserFactory(this.fileParserType, this.fileToReadRef);

          var extractedContent:Object = parser.parse();

          ExternalInterface.call("GEPBridge", {
              results: { data: extractedContent },
              event_name: "readcomplete"
          });

        } catch (error:Error) {
          ExternalInterface.call("GEPBridge", {
              results: { message: "Cannot read file: " + error.message },
              event_name: "readerror"
          });
        }
    }
